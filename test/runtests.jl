...
println("λ=0:  ",difActionAHO(n_tau,a,m,ω,λ,Path,i,x_new))
println("λ=24: ",difActionAHO(n_tau,a,m,ω,24.,Path,i,x_new))

@testset "difActionAHO" begin
    # Action is constant for x′ == x
    @test difActionAHO(n_tau,a,m,ω,λ,Path,i,Path[i]) ≈ 0. atol=1e-10
    # New difActionAHO equal to old procedure
    ...
    # AHO action not same as HO action limit
    @test difActionAHO(n_tau,a,m,ω,24.,Path,i,x_new) ≉  difActionAHO(n_tau,a,m,ω,λ,Path,i,x_new) atol=1e-10
